...
const CHROME_URL_REGEX = 'chrome:\\/\\/[a-z0-9]+(?:-[a-z0-9]+)*\\/?';
const USER_INFO_REGEX =
  "((([a-z]|\\d|-|\\.|_|~|[\\u00A0-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFEF])|(%[\\da-f]{2})|[!$&'()*+,;=]|:)*@)?";

const IP4_REGEX =
  '((?!0|10\\.|127\\.|169\\.254\\.|172\\.(?:1[6-9]|2[0-9]|3[0-1])\\.|192\\.0\\.2\\.|192\\.88\\.99\\.|192\\.168\\.|198\\.18\\.|198\\.51\\.100\\.|203\\.0\\.113\\.|224\\.|240\\.|255\\.)(?:25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9]?[0-9])(?:\\.(?:25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9]?[0-9])){3})|0\\.0\\.0\\.0';
...
const DOMAIN_REGEX =
  '(?:(?:xn--[a-z0-9]+(?:-[a-z0-9]+)*|[a-z\\u00a1-\\uffff0-9]+(?:-[a-z\\u00a1-\\uffff0-9]+)*)(?:\\.[a-z\\u00a1-\\uffff0-9]+(?:-[a-z\\u00a1-\\uffff0-9]+)*)*(?:\\.[a-z\\u00a1-\\uffff]{1,}))';
...
const completeRegexString = `((${PROTOCOL_REGEX}(${USER_INFO_REGEX}(${IP4_REGEX}|${IP6_REGEX}|localhost|${DOMAIN_REGEX})${PORT_REGEX}${PATH_REGEX}${QUERY_REGEX}${FRAGMENT_REGEX})|${CHROME_URL_REGEX})`;

const REGEXP_URL = new RegExp(completeRegexString, 'gi');

const REGEXP_EMAIL =
  /(\/|:)?(?=[A-Za-z0-9-_])([A-Za-z0-9_'+\-.]{1,63}[A-Za-z0-9_+-])@([A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)*\.[A-Za-z]{2,63}/g;
